...
class NetworkAnalyzer {
  ...
  static Stream<NetworkAddress> discover2(
    ...
    for (int i = 1; i < 256; ++i) {
      final host = '$subnet.$i';
      final Future<Socket> f = _ping(host, port, timeout, i ~/ 50);
      futures.add(f);
      f.then((socket) {
        socket.destroy();
  ...
  static Future<Socket> _ping(
      String host, int port, Duration timeout, int groupIndex) {
    return Future.delayed(Duration(seconds: groupIndex * timeout.inSeconds))
        .then(
      (value) => Socket.connect(host, port, timeout: timeout).then(
        (socket) {
          return socket;
        },
      ),
